...
/**
 * The ReceiveTaskParseListener class is a listener that gets triggered during the parsing of a BPMN process.
 * Specifically, it is triggered when a receive task is encountered in the process.
 *
 * @author Yusuf Bozkurt
 */
public class ReceiveTaskParseListener extends AbstractBpmnParseListener {

  /**
   * Method invoked when a receive task is encountered during the parsing of a BPMN process.
   * It initializes a {@link ReceiveTaskListener ReceiveTaskListener} and adds it to the receive task.
  ...
  public void parseReceiveTask(Element userTaskElement, ScopeImpl scope, ActivityImpl activity) {
    ReceiveTaskListener listener = new ReceiveTaskListener();
